...
function startMiniTickerMonitor(monitorId, broadcastLabel, logs) {
    ...
    exchange.miniTickerStream(async (markets) => {
        ...
        try {
            ...
            const books = Object.entries(markets).map(mkt => {
                const book = { symbol: mkt[0], bestAsk: Number(mkt[1].close), bestBid: Number(mkt[1].close) };
    ...
    if (logs) logger('M:' + monitorId, 'Mini Ticker Monitor has started!');
}
...
function startBookMonitor(monitorId, broadcastLabel, logs) {
    ...
    if (logs) logger('M:' + monitorId, 'Book Monitor has started!');
}
...
console.log(order)
                ...
                if (order.side == 'BUY' && order.status == 'FILLED'){
                    ...
                    let newPrice, factor;
                    newPrice = order.avgPrice * 1.0103;
...
function startChartMonitor(monitorId, symbol, interval, indexes, broadcastLabel, logs) {
    ...
    exchange.chartStream(symbol, interval || '1m', async (ohlc) => {
        ...
        try {
            ...
            beholder.updateMemory(symbol, indexKeys.P_PREVIOUS_CANDLE, interval, { current: previousPreviousCandle, previous: previousPreviousPreviousCandle }, false);
            ...
            if (results) {
                ...
                results.push(await beholder.testAutomations(beholder.parseMemoryKey(symbol, indexKeys.P_PREVIOUS_CANDLE, interval)));
    ...
    if (logs) logger('M:' + monitorId, `Chart Monitor has started for ${symbol}_${interval}!`);
}
...
function stopChartMonitor(monitorId, symbol, interval, indexes, logs) {
    ...
    beholder.deleteMemory(symbol, indexKeys.P_PREVIOUS_CANDLE, interval);
...
async function startTickerMonitor(monitorId, symbol, broadcastLabel, logs) {
    ...
    if (logs) logger('M:' + monitorId, `Ticker Monitor has started for ${symbol}`);
}
